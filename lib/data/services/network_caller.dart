...
class NetworkCaller {
  Future<ResponseData> getRequest(String url, {String? token}) async {
    ...
    final Response response =
        await get(Uri.parse(url), headers: {'token': token.toString(), 'Content-type': 'application/json'});
  ...
  Future<ResponseData> postRequest(String url, {String? token, Map<String, dynamic>? body}) async {
    ...
    final Response response = await post(Uri.parse(url),
        headers: {'token': token.toString(), 'Content-type': 'application/json'}, body: jsonEncode(body));
    log(response.statusCode.toString());
    log(response.body.toString());
    if (response.statusCode == 200) {
      final decodedResponse = jsonDecode(response.body);
